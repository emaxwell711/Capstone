...
public class Polygons {
    ...
    public boolean inPolygon(float[][] polygon, float[] point, int width) {
        ...
        do {
            ...
            // Check if the line segment from 'p' to 'extreme' intersects with the line segment from
            // 'polygon[i]' to 'polygon[next]'
            if (doIntersect(polygon[i], polygon[next], point, extreme)) {
                // If the point 'p' is colinear with line segment 'i-next', then check if it lies on
                // segment. If it does, return true. Otherwise, return false.
    ...
    // Given three colinear points p, q, and r, the function checks if point q lies on line segment
    // 'pr'
